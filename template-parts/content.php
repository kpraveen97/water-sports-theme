...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?> <?php event_listing_do_microdata('article'); ?>>
	...
	<div class="entry-content">
        ...
        if(is_singular()) {
            the_content(sprintf(
                ...
                /* translators: %s: Name of current post. Only visible to screen readers */
                    __('Continue reading<span class="screen-reader-text"> "%s"</span>', 'event-listing-water-sports'),
                    array(
                        'span' => array(
                            'class' => array(),
                        ),
		...
		wp_link_pages( array(
			'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'event-listing-water-sports' ),
			'after'  => '</div>',
		) );
		?>
	</div><!-- .entry-content -->
    ...
    if(is_singular()) {
        ...
        <div class="read-more-btn">
            <a href="<?php the_permalink(); ?>" class="btn"> <?php _e('Read More', 'event-listing-water-sports' ); ?></a>
        </div>
    <?php
    }
    ?>
